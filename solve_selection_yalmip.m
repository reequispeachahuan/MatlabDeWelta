...
objective_matrix = eye(2 * Nt) + ...
    (1 / lambda) * (sigma_x^2 / 2) * ...
    (H_eff_r_q_full' * diag(delta) * H_eff_r_q_full);

constraints = [
    delta(1:2*Nr) == 1
    0 <= delta <= 1
    sum(delta) == 2 * Nr + alpha
    objective_matrix >= 1e-9 * eye(2 * Nt)
];

options = sdpsettings('solver', 'sdpt3', 'verbose', 0);

diagnostics = optimize(constraints, -0.5 * logdet(objective_matrix), options);
